...
```{r}
lm_out <- lm(formula = hdi ~ efree + gdp_01, data = ecfree_hdi_wb_data)
...
```{r}
ecfree_hdi_wb_data %>% filter(name %in% c('Brazil', 'United States', 'China', 'Argentina')) %>% filter(!is.na(SI.POV.GINI)) %>% arrange(country, year) %>%
  ggplot(aes( y = hdi, x = gini, colour = country)) + 
  geom_text(check_overlap = TRUE, aes(label = `index year`)) + 
  labs(y = 'HDI', x = 'GINI') +
  geom_point() + 
  geom_path()
...
```{r}
ecfree_hdi_wb_data %>%
  filter(name == 'Ireland') #%>% select(gni_01, hdi_01, efree_01)
```
